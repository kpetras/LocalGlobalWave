...
for sub,filePath in enumerate(fileList):
    ...
    x = waveData.get_data('EEG')[:, :, 500:750]#last second of stimulus
    ...
    from scipy.signal import welch
    dataPreStim = waveData.get_data('EEG')[:, :, 0:250]
    dataPostStim = waveData.get_data('EEG')[:, :, 500:750]
    ...
    waveData.save_to_file(savefolder + '/EEG_18_OpticalFlowAfterFilter_Hilbert_masked')
...
# Save to CSV
df.to_csv('theta_alpha_values.csv', index=False)
