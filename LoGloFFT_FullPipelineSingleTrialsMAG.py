...
fileName = "MEG_Mag_WaveData"
fileList = glob.glob(os.path.join(folder, "**", fileName), recursive=True)
#fileList= fileList[11:] #[KP] just for testing, remove later!
for sub,filePath in enumerate(fileList):
    ...
    nChans = waveData.get_data('Mag').shape[1]    
    evoked_dict = {}
    ...
    x = waveData.get_data('Mag')[:, :, 500:750]#last second of stimulus
    ...
    from scipy.signal import welch
    dataPreStim = waveData.get_data('Mag')[:, :, 0:250]
    dataPostStim = waveData.get_data('Mag')[:, :, 500:750]
    ...
    for freqInd in range(2):
        ...
        filt.filter_narrowband(waveData, dataBucketName = "Mag", LowCutOff=freq-1, HighCutOff=freq+1, type = "FIR", order=100, causal=False)
    ...
    # ani.save('OpticalFlowAfterFilter_Hilbert.gif')

    waveData.save_to_file(savefolder + '/Mag_18_OpticalFlowAfterFilter_Hilbert_masked')
